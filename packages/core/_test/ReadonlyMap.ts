...
import { fromEquals, fromOrd } from '@fp-ts/core/Eq'
...
import * as _ from '@fp-ts/core/ReadonlyMap'
import type { Refinement } from '@fp-ts/core/Refinement'
import type { Result } from '@fp-ts/core/Result'
import { fail, succeed } from '@fp-ts/core/Result'
import * as Se from '@fp-ts/core/Semigroup'
import * as Sh from '@fp-ts/core/Show'
...
describe('ReadonlyMap', () => {
  ...
  it('partitionMap', () => {
    const empty = new Map<string, number>()
    const f = (n: number) => (p(n) ? succeed(n + 1) : fail(n - 1))
  ...
  it('separate', () => {
    const fooBar = new Map<string, Result<number, number>>([
      ['foo', fail(123)],
      ['bar', succeed(123)]
    ])
  ...
  describe('getTraversableWithIndex', () => {
    ...
    it('traverseWithIndex should sort the keys', () => {
      ...
      const append = (message: string): IO.Sync<void> =>
        () => {
          log.push(message)
        }
  ...
  describe('getFilterableKind', () => {
    ...
    it('partitionMapKind', async () => {
      ...
      const f = (n: number) => T.of(p(n) ? succeed(n + 1) : fail(n - 1))
  ...
  describe('getFilterableWithIndex', () => {
    it('partitionMapWithIndex', () => {
      ...
      const f = (_: string, n: number) => (p(n) ? succeed(n + 1) : fail(n - 1))
