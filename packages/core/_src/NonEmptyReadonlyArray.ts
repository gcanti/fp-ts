 ...
 * @internal
 */
export const prepend = <B>(head: B) => <A>(tail: ReadonlyArray<A>): NonEmptyReadonlyArray<A | B> => [head, ...tail]

/**
 * @internal
 */
export const append = <B>(end: B) => <A>(init: ReadonlyArray<A>): NonEmptyReadonlyArray<A | B> => concat([end])(init)
 ...
 *
 * @example
 * import { comprehension } from '@fp-ts/core/NonEmptyReadonlyArray'
...
export const zip = <B>(bs: NonEmptyReadonlyArray<B>) =>
  <A>(as: NonEmptyReadonlyArray<A>): NonEmptyReadonlyArray<readonly [A, B]> => pipe(as, zipWith(bs, tuple.tuple))
...
) => <A>(afb: (a: A) => NonEmptyReadonlyArray<B>) => (a: A) => NonEmptyReadonlyArray<C> = flattenable
 ...
 * @since 3.0.0
 */
export const duplicate: <A>(ma: NonEmptyReadonlyArray<A>) => NonEmptyReadonlyArray<NonEmptyReadonlyArray<A>> = extend(
  identity
)
 ...
 * @since 3.0.0
 */
export const flatten: <A>(mma: NonEmptyReadonlyArray<NonEmptyReadonlyArray<A>>) => NonEmptyReadonlyArray<A> = flatMap(
  identity
)
...
export const mapWithIndex: <A, B>(
  f: (i: number, a: A) => B
) => (fa: NonEmptyReadonlyArray<A>) => NonEmptyReadonlyArray<B> = <A, B>(f: (i: number, a: A) => B) =>
  (as: NonEmptyReadonlyArray<A>): NonEmptyReadonlyArray<B> => {
 ...
 * @since 3.0.0
 */
export const traverseWithIndex = <F extends TypeLambda>(F: apply.Apply<F>) =>
 ...
 * @since 3.0.0
 */
export const flap: <A>(a: A) => <B>(fab: NonEmptyReadonlyArray<(a: A) => B>) => NonEmptyReadonlyArray<B> = functor.flap(
  Functor
)
 ...
 * @since 3.0.0
 */
export const as: <B>(b: B) => (self: NonEmptyReadonlyArray<unknown>) => NonEmptyReadonlyArray<B> = functor
  .as(Functor)
 ...
 * @since 3.0.0
 */
export const unit: (self: NonEmptyReadonlyArray<unknown>) => NonEmptyReadonlyArray<void> = functor.unit(
  Functor
)
...
export const lift2: <A, B, C>(
  f: (a: A, b: B) => C
) => (fa: NonEmptyReadonlyArray<A>, fb: NonEmptyReadonlyArray<B>) => NonEmptyReadonlyArray<C> = apply
  .lift2(Apply)
 ...
 * @since 3.0.0
 */
export const reduce: <B, A>(b: B, f: (b: B, a: A) => B) => (self: NonEmptyReadonlyArray<A>) => B = foldable.reduce(
  Foldable
)
...
export const foldMap = <S>(S: Semigroup<S>) =>
  <A>(f: (a: A) => S) =>
    (self: NonEmptyReadonlyArray<A>): S => self.slice(1).reduce((s, a) => S.combine(f(a))(s), f(self[0]))

/**
 * @category folding
 * @since 3.0.0
 */
export const reduceRight: <B, A>(b: B, f: (a: A, b: B) => B) => (self: NonEmptyReadonlyArray<A>) => B = foldable
...
export const foldMapWithIndex = <S>(S: Semigroup<S>) =>
  <A>(f: (i: number, a: A) => S) =>
    (self: NonEmptyReadonlyArray<A>): S => self.slice(1).reduce((s, a, i) => S.combine(f(i + 1, a))(s), f(0, self[0]))
...
) => (self: NonEmptyReadonlyArray<A>) => B = foldableWithIndex.reduceRightWithIndex(FoldableWithIndex)
 ...
 * @since 3.0.0
 */
export const Do: NonEmptyReadonlyArray<{}> = of(_.emptyReadonlyRecord)
...
export const bindTo: <N extends string>(
  name: N
) => <A>(self: NonEmptyReadonlyArray<A>) => NonEmptyReadonlyArray<{ readonly [K in N]: A }> = functor
  .bindTo(Functor)
...
) => NonEmptyReadonlyArray<{ readonly [K in N | keyof A]: K extends keyof A ? A[K] : B }> = functor.let(
  Functor
)
...
export const bind: <N extends string, A extends object, B>(
  ...
  self: NonEmptyReadonlyArray<A>
) => NonEmptyReadonlyArray<{ readonly [K in keyof A | N]: K extends keyof A ? A[K] : B }> = flattenable
  .bind(Flattenable)
 ...
 * @since 3.0.0
 */
export const Zip: NonEmptyReadonlyArray<readonly []> = of(_.emptyReadonlyArray)

/**
 * @category tuple sequencing
 * @since 3.0.0
 */
export const tupled: <A>(self: NonEmptyReadonlyArray<A>) => NonEmptyReadonlyArray<readonly [A]> = functor
  .tupled(Functor)
