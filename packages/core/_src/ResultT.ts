...
export const succeed = <F extends TypeLambda>(FromIdentity: FromIdentity<F>) =>
  <A, S>(a: A): Kind<ResultT<F, never>, S, unknown, never, never, A> => FromIdentity.of(result.succeed(a))

/**
 * @since 3.0.0
 */
export const fail = <F extends TypeLambda>(FromIdentity: FromIdentity<F>) =>
  <E, S>(e: E): Kind<ResultT<F, E>, S, unknown, never, never, never> => FromIdentity.of(result.fail(e))
 ...
 * @since 3.0.0
 */
export const getValidatedOrElse = <F extends TypeLambda, E>(Monad: Monad<F>, Semigroup: Semigroup<E>) =>
...
export const getOrElseKind = <F extends TypeLambda>(Monad: Monad<F>) =>
  <S, R2, O2, FE2, B>(onError: Kind<F, S, R2, O2, FE2, B>) =>
    <R1, O1, FE1, A>(
      ...
      return pipe(
        self,
        Monad.flatMap(result.match<unknown, Kind<F, S, R2, O2, FE2, A | B>, A>(() => onError, Monad.of))
      )
    }
