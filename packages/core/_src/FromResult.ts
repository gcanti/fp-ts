...
export const liftPredicate: <F extends TypeLambda>(
  ...
  <B extends A, E, A = B>(predicate: Predicate<A>, onFalse: E) =>
    <S>(b: B): Kind<F, S, unknown, never, E, B> => FromResult.fromResult(predicate(b) ? _.succeed(b) : _.fail(onFalse))
 ...
 * @since 3.0.0
 */
export const filterMap = <F extends TypeLambda>(FromResult: FromResult<F>, Flattenable: Flattenable<F>) =>
 ...
 * @since 3.0.0
 */
export const partitionMap = <F extends TypeLambda>(FromResult: FromResult<F>, Flattenable: Flattenable<F>) =>
