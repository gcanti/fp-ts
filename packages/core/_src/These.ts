...
import type * as fromThese_ from '@fp-ts/core/FromThese'
 ...
 * @since 3.0.0
 */
export const failureOrBoth = <E>(e: E) => <A>(ma: Option<A>): These<E, A> => _.isNone(ma) ? fail(e) : both(e, ma.value)
 ...
 * @since 3.0.0
 */
export const match = <E, B, A, C = B, D = B>(onError: (e: E) => B, onSuccess: (a: A) => C, onBoth: (e: E, a: A) => D) =>
  (fa: These<E, A>): B | C | D => {
    switch (fa._tag) {
      case 'Failure':
 ...
 * @since 3.0.0
 */
export const mapBoth: <E, G, A, B>(f: (e: E) => G, g: (a: A) => B) => (self: These<E, A>) => These<G, B> = (f, g) =>
  (fa) =>
 ...
 * @since 3.0.0
 */
export const fromOption: <E>(onNone: E) => <A>(fa: Option<A>) => These<E, A> = fromResult_.fromOption(
  FromResult
)
 ...
 * @since 3.0.0
 */
export const reduce: <B, A>(b: B, f: (b: B, a: A) => B) => <E>(self: These<E, A>) => B = foldable.reduce(
  Foldable
)
 ...
 * @since 3.0.0
 */
export const foldMap: <M>(Monoid: Monoid<M>) => <A>(f: (a: A) => M) => <E>(self: These<E, A>) => M = foldable.foldMap(
  Foldable
)
 ...
 * @since 3.0.0
 */
export const reduceRight: <B, A>(b: B, f: (a: A, b: B) => B) => <E>(self: These<E, A>) => B = foldable
 ...
 *
 * @example
 * import { getFailure, fail, succeed, both } from '@fp-ts/core/These'
 * import { none, some } from '@fp-ts/core/Option'
 *
 ...
 *
 * @example
 * import { getSuccess, fail, succeed, both } from '@fp-ts/core/These'
 * import { none, some } from '@fp-ts/core/Option'
 *
 ...
 * @since 3.0.0
 */
export const Zip: These<never, readonly []> = succeed(_.emptyReadonlyArray)
