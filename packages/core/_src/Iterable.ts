...
export const foldMap = <M>(Monoid: Monoid<M>) =>
  <A>(f: (a: A) => M): ((self: Iterable<A>) => M) => reduce(Monoid.empty, (b, a) => Monoid.combine(f(a))(b))
...
export const reduceRight = <B, A>(b: B, f: (a: A, b: B) => B) =>
  (self: Iterable<A>): B => Array.from(self).reduceRight((b, a) => f(a, b), b)

/**
 * @category folding
 * @since 3.0.0
 */
export const reduceWithIndex = <B, I, A>(b: B, f: (i: I, b: B, a: A) => B) =>
 ...
 * @since 3.0.0
 */
export const reduceRightWithIndex = <B, I, A>(b: B, f: (i: I, a: A, b: B) => B) =>
  (self: Iterable<readonly [I, A]>): B => Array.from(self).reduceRight((b, [i, a]) => f(i, a, b), b)
 ...
 *
 * @example
 * import { intercalate } from '@fp-ts/core/Iterable'
 * import { Monoid } from '@fp-ts/core/string'
 * import * as T from '@fp-ts/core/Tree'
 * import { pipe } from '@fp-ts/core/Function'
 *
 ...
 * @since 3.0.0
 */
export const intercalate = <M>(Monoid: Monoid<M>) =>
  (separator: M) =>
    (self: Iterable<M>) => {
 ...
 *
 * @example
 * import { reduceKind } from '@fp-ts/core/Iterable'
 * import { Flattenable, some } from '@fp-ts/core/Option'
 * import * as T from '@fp-ts/core/Tree'
 * import { pipe } from '@fp-ts/core/Function'
 *
 ...
 * @since 3.0.0
 */
export const reduceKind = <F extends TypeLambda>(Flattenable: Flattenable<F>) =>
  <S, R, O, E, B, A>(
    fb: Kind<F, S, R, O, E, B>,
