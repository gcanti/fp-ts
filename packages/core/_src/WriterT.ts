 ...
 * @since 3.0.0
 */
export const fromKind = <F extends TypeLambda>(Functor: Functor<F>) =>
 ...
 * @since 3.0.0
 */
export const fromSync = <F extends TypeLambda>(Functor: Functor<F>, FromSync: FromSync<F>) =>
  <W>(w: W) =>
 ...
 * @since 3.0.0
 */
export const fromAsync = <F extends TypeLambda>(Functor: Functor<F>, FromAsync: FromAsync<F>) =>
  <W>(w: W) =>
 ...
 * @since 3.0.0
 */
export const tell = <F extends TypeLambda>(FromIdentity: FromIdentity<F>) =>
  <W, S>(w: W): Kind<WriterT<F, W>, S, unknown, never, never, void> => FromIdentity.of(writer.tell(w))
 ...
 * @since 3.0.0
 */
export const of = <F extends TypeLambda, W>(FromIdentity: FromIdentity<F>, Monoid: Monoid<W>) =>
  <A, S>(a: A): Kind<WriterT<F, W>, S, unknown, never, never, A> => FromIdentity.of([Monoid.empty, a])
 ...
 * @since 3.0.0
 */
export const flatMap = <F extends TypeLambda, W>(Flattenable: Flattenable<F>, Semigroup: Semigroup<W>) =>
  <A, S, R1, FO1, E1, B>(
 ...
 * @since 3.0.0
 */
export const mapLeft = <F extends TypeLambda>(Functor: Functor<F>) =>
  <W, X>(
    f: (w: W) => X
...
// TODO: orElse, none, fromResult, fromReader, fromState, reduce, foldMap, reduceRight, traverse, contramap
 ...
 * @since 3.0.0
 */
export const listens = <F extends TypeLambda>(Functor: Functor<F>) =>
  <W, B>(
    f: (w: W) => B
 ...
 * @since 3.0.0
 */
export const censor = <F extends TypeLambda>(Functor: Functor<F>) =>
  <W>(
    f: (w: W) => W
