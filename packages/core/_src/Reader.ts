...
import type * as fromReader_ from '@fp-ts/core/FromReader'
...
import type * as profunctor from '@fp-ts/core/Profunctor'
 ...
 * @since 3.0.0
 */
export const flatMap: <A, R2, B>(f: (a: A) => Reader<R2, B>) => <R1>(self: Reader<R1, A>) => Reader<R1 & R2, B> = (f) =>
  (self) => (r) => f(self(r))(r)
...
export const composeKleisli: <B, R2, C>(
  bfc: (b: B) => Reader<R2, C>
) => <A, R1>(afb: (a: A) => Reader<R1, B>) => (a: A) => Reader<R1 & R2, C> = flattenable.composeKleisli(
  Flattenable
)
...
export const lift3: <A, B, C, D>(
  f: (a: A, b: B, c: C) => D
) => <R1, R2, R3>(fa: Reader<R1, A>, fb: Reader<R2, B>, fc: Reader<R3, C>) => Reader<R1 & R2 & R3, D> = apply.lift3(
  Apply
)
 ...
 * @since 3.0.0
 */
export const Do: Reader<unknown, {}> = of(_.emptyReadonlyRecord)
...
const let_: <N extends string, A extends object, B>(
  ...
  f: (a: A) => B
) => <R>(self: Reader<R, A>) => Reader<R, { readonly [K in N | keyof A]: K extends keyof A ? A[K] : B }> = functor.let(
  Functor
)
...
export const bindRight: <N extends string, A extends object, R2, B>(
  name: Exclude<N, keyof A>,
  fb: Reader<R2, B>
) => <R1>(self: Reader<R1, A>) => Reader<R1 & R2, { readonly [K in keyof A | N]: K extends keyof A ? A[K] : B }> = apply
  .bindRight(Apply)
 ...
 * @since 3.0.0
 */
export const Zip: Reader<unknown, readonly []> = of(_.emptyReadonlyArray)
...
export const zipFlatten: <R2, B>(
  fb: Reader<R2, B>
) => <R1, A extends ReadonlyArray<unknown>>(self: Reader<R1, A>) => Reader<R1 & R2, readonly [...A, B]> = apply
  .zipFlatten(Apply)
 ...
 * @since 3.0.0
 */
export const traverseNonEmptyReadonlyArrayWithIndex = <A, R, B>(f: (index: number, a: A) => Reader<R, B>) =>
