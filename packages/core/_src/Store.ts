 ...
 * @since 3.0.0
 */
import type { Comonad as Comonad_ } from '@fp-ts/core/Comonad'
 ...
 * @since 3.0.0
 */
export const map: <A, B>(f: (a: A) => B) => <S>(fa: Store<S, A>) => Store<S, B> = (f) =>
  (fa) => ({
    peek: (s) => f(fa.peek(s)),
    pos: fa.pos
  })
 ...
 * @since 3.0.0
 */
export const extend: <S, A, B>(f: (wa: Store<S, A>) => B) => (wa: Store<S, A>) => Store<S, B> = (f) =>
  (wa) => ({
 ...
 * @since 3.0.0
 */
export const duplicate: <S, A>(wa: Store<S, A>) => Store<S, Store<S, A>> = extend(identity)
 ...
 * @since 3.0.0
 */
export const flap: <A>(a: A) => <S, B>(fab: Store<S, (a: A) => B>) => Store<S, B> = functor.flap(Functor)
