 ...
 * @since 3.0.0
 */
export const chainFirst = <M extends HKT>(M: Chain<M>) => <A, S, R, E, B>(
  f: (a: A) => Kind<M, S, R, E, B>
): ((first: Kind<M, S, R, E, A>) => Kind<M, S, R, E, A>) =>
  M.chain((a) =>
    pipe(
      f(a),
      M.map(() => a)
 ...
 * @since 3.0.0
 */
export const bind = <M extends HKT>(M: Chain<M>) => <N extends string, A, S, R, E, B>(
  name: Exclude<N, keyof A>,
  f: <A2 extends A>(a: A | A2) => Kind<M, S, R, E, B>
): ((ma: Kind<M, S, R, E, A>) => Kind<M, S, R, E, { readonly [K in keyof A | N]: K extends keyof A ? A[K] : B }>) =>
  M.chain((a) =>
    pipe(
      f(a),
