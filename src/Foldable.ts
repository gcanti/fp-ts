...
export function foldMap<F extends HKTS, M>(
  foldable: Foldable<F>,
  monoid: Monoid<M>
): <A>(f: (a: A) => M, fa: HKT<A>[F]) => M {
  return <A>(f: (a: A) => M, fa: HKT<A>[F]) =>
    foldable.reduce((acc, x: A) => monoid.concat(acc, f(x)), monoid.empty(), fa)
}
 ...
 */
export function getCompositionFoldable<FG extends HKTS, F extends HKTS, G extends HKTS>(
  URI: FG,
...
export function intercalate<F extends HKTS, M>(foldable: Foldable<F>, monoid: Monoid<M>): (sep: M, fm: HKT<M>[F]) => M {
  ...
  return (sep, fm) => {
    function go({ init, acc }: Acc, x: M): Acc {
...
export function traverse_<M extends HKTS, F extends HKTS>(
  applicative: Applicative<M>,
  foldable: Foldable<F>
): <A, B, U = any, V = any>(f: (a: A) => HKT<B, U, V>[M], fa: HKT<A, U, V>[F]) => HKT<void, U, V>[M] {
  return <A, B>(f: (a: A) => HKT<B>[M], fa: HKT<A>[F]) =>
    toArray(foldable)<A>(fa).reduce(
      (mu, a) => applyFirst(applicative)<undefined, B>(mu, f(a)),
      applicative.of(undefined)
    )
}

export function sequence_<M extends HKTS, F extends HKTS>(
  applicative: Applicative<M>,
  foldable: Foldable<F>
): <A, U = any, V = any>(fa: HKT<A, U, V>[F]) => HKT<void, U, V>[M] {
