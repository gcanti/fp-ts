...
export function chainF<M extends URIS3, ME>(
  M: Monad3C<M, ME>
...
export function chainF<M extends URIS2, ME>(
  M: Monad2C<M, ME>
...
export function chainF<M>(
  M: Monad<M>
): <A, E, B>(f: (a: A) => HKT<M, B>) => (ma: HKT<M, Either<E, A>>) => HKT<M, Either<E, B>>
export function chainF<M>(
  M: Monad<M>
): <A, E, B>(f: (a: A) => HKT<M, B>) => (ma: HKT<M, Either<E, A>>) => HKT<M, Either<E, B>> {
  return (f) => chain(M)(flow(f, (fa) => M.map(fa, (i) => E.of(i))))
}
