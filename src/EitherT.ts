...
export function right<F extends URIS2>(F: Functor<F>): <L, M, A>(fa: HKT2<F, M, A>) => Type2<F, M, Either<L, A>>
export function right<F extends URIS>(F: Functor<F>): <L, A>(fa: HKT<F, A>) => Type<F, Either<L, A>>
export function right<F>(F: Functor<F>): <L, A>(fa: HKT<F, A>) => HKT<F, Either<L, A>>
export function right<F>(F: Functor<F>): <L, A>(fa: HKT<F, A>) => HKT<F, Either<L, A>> {
  return ma => F.map(ma, a => either.right(a))
}

export function left<F extends URIS2>(F: Functor<F>): <L, M, A>(fl: HKT2<F, M, L>) => Type2<F, M, Either<L, A>>
export function left<F extends URIS>(F: Functor<F>): <L, A>(fl: HKT<F, L>) => Type<F, Either<L, A>>
export function left<F>(F: Functor<F>): <L, A>(fl: HKT<F, L>) => HKT<F, Either<L, A>>
export function left<F>(F: Functor<F>): <L, A>(fl: HKT<F, L>) => HKT<F, Either<L, A>> {
  return ml => F.map(ml, l => either.left(l))
}

export function fromEither<F extends URIS2>(F: Applicative<F>): <L, M, A>(fa: Either<L, A>) => Type2<F, M, Either<L, A>>
export function fromEither<F extends URIS>(F: Applicative<F>): <L, A>(fa: Either<L, A>) => Type<F, Either<L, A>>
...
export function getEitherT<M extends URIS2>(M: Monad<M>): EitherT2<M>
export function getEitherT<M extends URIS>(M: Monad<M>): EitherT1<M>
