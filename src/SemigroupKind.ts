/**
 * `SemigroupKind` has a very similar structure to `Semigroup`, the difference is that `SemigroupKind` operates on type
 * constructors of one argument. So, for example, whereas you can find a `Semigroup` for types which are fully
 * specified like `number` or `ReadonlyArray<number>` or `Option<number>`, you will find `SemigroupKind` for type constructors like `ReadonlyArray` and `Option`.
 ...
 * and, in fact, the `K` in `SemigroupKind` stands for `Kind`.
 *
 * `SemigroupKind` instances are required to satisfy the following laws:
 *
