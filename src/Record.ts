 ...
 * @since 1.12.0
 */
export const reduceWithKey = <A, B>(fa: { [key: string]: A }, b: B, f: (k: string, b: B, a: A) => B): B => {
  let out: B = b
 ...
 * @since 1.12.0
 */
export const foldMapWithKey = <M>(M: Monoid<M>) => <A>(fa: { [key: string]: A }, f: (k: string, a: A) => M): M => {
  let out: M = M.empty
  const keys = Object.keys(fa).sort()
  const len = keys.length
  for (let i = 0; i < len; i++) {
 ...
 * @since 1.12.0
 */
export const foldrWithKey = <A, B>(fa: { [key: string]: A }, b: B, f: (k: string, a: A, b: B) => B): B => {
  let out: B = b
  const keys = Object.keys(fa).sort()
  const len = keys.length
