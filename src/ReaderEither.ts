...
) => <R1>(ma: Reader<R1, E.Either<E, A>>) => Reader<R1 & R2 & R3, B | C> = /*#__PURE__*/ ET.matchE(R.Monad)
 ...
 * @since 3.0.0
 */
export const getApplicativeReaderValidation = <E>(S: Semigroup<E>): Applicative_<ReaderEitherFFixedE<E>> => ({
  map,
 ...
 * @since 3.0.0
 */
export const getAltReaderValidation = <E>(S: Semigroup<E>): Alt_<ReaderEitherFFixedE<E>> => {
  return {
    map,
 ...
 * @since 3.0.0
 */
export const getCompactable = <E>(M: Monoid<E>): Compactable<ReaderEitherFFixedE<E>> => {
 ...
 * @since 3.0.0
 */
export const getFilterable = <E>(M: Monoid<E>): Filterable<ReaderEitherFFixedE<E>> => {
...
export const chainFirstReaderK: <A, R2, B>(
  f: (a: A) => Reader<R2, B>
) => <R1, E = never>(ma: ReaderEither<R1, E, A>) => ReaderEither<R1 & R2, E, A> = /*#__PURE__*/ chainFirstReaderK_(
  FromReader,
  Chain
)
...
) => ReaderEither<R, E, { readonly [K in N | keyof A]: K extends keyof A ? A[K] : B }> = /*#__PURE__*/ let__(Functor)
...
export const apS: <N extends string, A, R2, E2, B>(
  ...
  fa: ReaderEither<R1, E1, A>
) => ReaderEither<R1 & R2, E1 | E2, { readonly [K in keyof A | N]: K extends keyof A ? A[K] : B }> = /*#__PURE__*/ apS_(
  Apply
)
...
) => ReaderEither<R1 & R2, E1 | E2, readonly [...A, B]> = /*#__PURE__*/ apT_(Apply)
