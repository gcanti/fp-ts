...
import { tuple, pipeOp } from '../src/function'
import { monoidString } from '../src/Monoid'

describe('Writer', () => {
  it('eval', () => {
    const fa = new Writer(() => [1, 'a'])
    assert.strictEqual(pipeOp(fa, evalWriter), 1)
  })

  it('exec', () => {
    const fa = new Writer(() => [1, 'a'])
    assert.strictEqual(pipeOp(fa, execWriter), 'a')
  })
  ...
  it('listens', () => {
    const fa = new Writer(() => [1, 'a'])
    assert.deepStrictEqual(pipeOp(fa, listens2v(w => w.length)).run(), [[1, 1], 'a'])
  })

  it('censor', () => {
    const fa = new Writer(() => [1, ['a', 'b']])
    assert.deepStrictEqual(pipeOp(fa, censor2v(w => w.filter(a => a !== 'a'))).run(), [1, ['b']])
  })
