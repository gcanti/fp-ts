...
describe('Filterable', () => {
  it('getFilterableComposition', () => {
    ...
    assert.deepStrictEqual(F.filter([[1, 2], [3, 4]], a => a > 1), [[2], [3, 4]])

    assert.deepStrictEqual(F.filterMap([['a', 'bb'], ['ccc', 'dddd']], a => (a.length > 1 ? some(a.length) : none)), [
      [2],
      [3, 4]
    ])

    assert.deepStrictEqual(F.partition([['a', 'bb'], ['ccc', 'dddd']], a => a.length % 2 === 0), {
      left: [['a'], ['ccc']],
      right: [['bb'], ['dddd']]
    })
