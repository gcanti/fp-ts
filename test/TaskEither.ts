...
describe('TaskEither', () => {
  ...
  describe('bracket', () => {
    ...
    const acquireSuccess = make({ res: 'acquire success' })
    const useSuccess = () => make('use success')
  ...
  it('chain', () => {
    const te1 = taskEither.chain(make('foo'), a => (a.length > 2 ? make(a.length) : fromLeft('foo')))
    const te2 = taskEither.chain(make('a'), a => (a.length > 2 ? make(a.length) : fromLeft('foo')))
  ...
  it('applySecond', () => {
    ...
    const append = (message: string, millis: number): TaskEither<string, number> =>
      right(T.task.map(T.delay(millis, undefined), () => log.push(message)))
    return append('a', 10)
  ...
  it('alt', () => {
    ...
    const r1: TaskEither<string, number> = make(1)
    const r2 = make(2)
    const x1 = l1.alt(l2)
    const x2 = l1.alt(r1)
    const x3 = r1.alt(l1)
  ...
  it('getSemigroup', () => {
    ...
    return Promise.all([
      S.concat(left(T.delay(10, 'a')), left(T.delay(10, 'b')))
        .run()
        .then(x => assert.deepStrictEqual(x, eitherLeft('a'))),
      S.concat(left(T.delay(10, 'a')), right(T.delay(10, 2)))
        .run()
        .then(x => assert.deepStrictEqual(x, eitherRight(2))),
      S.concat(right(T.delay(10, 1)), left(T.delay(10, 'b')))
        .run()
        .then(x => assert.deepStrictEqual(x, eitherRight(1))),
      S.concat(right(T.delay(10, 1)), right(T.delay(10, 2)))
        .run()
  ...
  describe('getApplyMonoid', () => {
    ...
    it('concat (right)', () => {
      return M.concat(right(T.delay(10, 'a')), right(T.delay(10, 'b')))
        .run()
        .then(x => assert.deepStrictEqual(x, eitherRight('ab')))
    })
    it('concat (left)', () => {
      return M.concat(right(T.delay(10, 'a')), left(T.delay(10, 'b')))
        .run()
        .then(x => assert.deepStrictEqual(x, eitherLeft('b')))
    })
    it('empty (right)', () => {
      return M.concat(right(T.delay(10, 'a')), M.empty)
        .run()
        .then(x => assert.deepStrictEqual(x, eitherRight('a')))
    })
    it('empty (left)', () => {
      return M.concat(M.empty, right(T.delay(10, 'a')))
        .run()
        .then(x => assert.deepStrictEqual(x, eitherRight('a')))
    })
  })
  ...
  it('sequence parallel', () => {
    const log: Array<string> = []
    const append = (message: string): TaskEither<void, number> => right(() => Promise.resolve(log.push(message)))
  ...
  it('foldTaskEither', () => {
    const whenLeft = (s: string) => (s.length >= 2 ? make('okleft') : fromLeft(false))
    const whenRight = (n: number) => (n >= 2 ? make('okright') : fromLeft(true))
  ...
  it('foldTask', () => {
    ...
    const tasks = [fromLeft('a').foldTask(whenLeft, whenRight)(), make(1).foldTask(whenLeft, whenRight)()]
  ...
  it('filterOrElse', () => {
    const isNumber = (u: string | number): u is number => typeof u === 'number'
    const actual = (make(12) as TaskEither<string, number>).filterOrElse(isNumber, 'not a number')
    const tasks = [
      (make(12) as TaskEither<string, number>).filterOrElse(n => n > 10, 'bar'),
      (make(7) as TaskEither<string, number>).filterOrElse(n => n > 10, 'bar'),
  ...
  it('filterOrElseL', () => {
    const isNumber = (u: string | number): u is number => typeof u === 'number'
    const actual = (make(12) as TaskEither<string, number>).filterOrElseL(isNumber, () => 'not a number')
    const tasks = [
      (make(12) as TaskEither<string, number>).filterOrElseL(n => n > 10, () => 'bar'),
      (make(7) as TaskEither<string, number>).filterOrElseL(n => n > 10, () => 'bar'),
      fromLeft('foo').filterOrElseL(n => n > 10, () => 'bar'),
      (make(7) as TaskEither<string, number>).filterOrElseL(n => n > 10, n => `invalid ${n}`),
      actual
    ]
  ...
  describe('MonadThrow', () => {
    it('should obey the law', () => {
      return Promise.all([
        taskEither.chain(taskEither.throwError('error'), a => make(a)).run(),
